...
import SelectInterSection from "./SelectInterSection/SelectInterSetion";
...
const BodyRapport2 = ({
  ...
  const handleUpload = async (
    ...
    setDataInter((prevState) =>
      prevState.map((data, indexx) => {
        if (indexx === indexData) {
  ...
  const handleChangeInfoImage = (e, index, indexImage, name) => {
    const newDataInterImage = dataInter.map((data, i) => {
      if (name === "epreuve" && index === i) {
        const neww = data.image.map((image, i) =>
          i === indexImage ? { ...image, [name]: e } : { ...image }
        );
  ...
  const handleChangeMoyen = (indexMoyen, index, name, moyenValue) => {
    const newDataMoyenUtilise = dataInter.map((data, i) => {
      if (index === i) {
        const newData = data.moyenTechnique.map((moyen, iMoyen) =>
          indexMoyen === iMoyen
            ? { ...moyen, [name]: !moyenValue }
            : { ...moyen }
        );
  ...
  return (
    <>
      {/* <ImageCanvas/> */}
      <div>
        <h2 className="title-intervention">Mon intervention</h2>
      </div>
          ...
          return (
             ...
             */}
              ...
              <div className="sectionImageBody">
                {data.image.length > 0 &&
                  data.image.map((image, indexImage) => (
                    <div key={indexImage}>
                      <div className="sectionInfoImage">
                        ...
                        {deleteImage && (
                          <ModalDeleteImage
                            infoInter={infoInter}
                            openModalImageDelete={openModalImageDelete}
            ...
            <div key={index} className="separation">
              ...
              <div className="bloc-btn-ChangePlaceSection">
                {index < 5 || index === 5 ? (
                  ...
                  <button
                    className="btnChangePlaceSection-up"
                    onClick={(e) => sectionMoveUp(e, index, data)}
                  >{`<`}</button>
                )}

                {index === dataInter.length - 1 || index < 5 ? (
                  ""
                ) : (
                  <button
                    className="btnChangePlaceSection-down"
                    onClick={(e) => sectionMoveDown(e, index, data)}
                  >{`>`}</button>
                )}
              </div>
              ...
              <div className="s-titre-des">
                <label className="titre-des">
                  ...
                  data.section === "miseEnPression" ? (
                    ...
                    <select
                      onChange={(e) =>
                        handleChangeInfoInter(e.target.value, index, "titre")
                      }
                    >
                      {dataInter
                        .filter((data) => data.section === "moyenTechnique")
                        .map((moyen) =>
                          moyen.moyenTechnique.map(
                            (materiel, indexMateriel) => (
                              <option
                                key={indexMateriel}
                                value={materiel.materielUtilise}
                              >
                ...
                <label>
                  <div> Description : </div>
                  <textarea
                    className="texttarea-description"
                    name="description"
                    type="textarea"
                    maxLength={
                      data.section === "conclusion" ||
                    ...
                    <div key={indexImage}>
                      <div className="sectionInfoImage">
                        ...
                        <div className="blocImageRapportBody">
                          ...
                          {zoomImage &&
                            indexImage === indexImageZoom &&
                            index === sectionIndexZoom && (
                              <ImageCanvas
                                setCanvasImageLoaded={setCanvasImageLoaded}
                                setContainFile={setContainFile}
                                dataInter={dataInter}
                                setDataInter={setDataInter}
                                index={index}
                                indexImage={indexImage}
                                urlImageZoom={image.url}
                                imageWidth={image.imageWidth}
                                imageHeight={image.imageHeight}
                                urlZoom={indexImageZoom}
                        ...
                        <div>
                          {indexImage === 0 || data.image.length === 1 ? (
                            ""
                          ) : (
                            <button
                              className="btnChangePlaceImage-up"
                              onClick={(e) =>
                                moveImageUp(e, index, indexImage, data.image)
                              }
                            >{`<`}</button>
                          )}

                          {indexImage === data.image.length - 1 ||
                          data.image.length === 1 ? (
                            ""
                          ) : (
                            <button
                              className="btnChangePlaceImage-down"
                              onClick={(e) =>
                                moveImageDown(e, index, indexImage, data.image)
                              }
                            >{`>`}</button>
                          )}
                        </div>
                        ...
                        {data.section === "miseEnPression" ? (
                          <div className="section-misePression" key={index}>
                            ...
                            <label className="labelMiseEnPression">
                              Réseau eau :
                              <textarea
                                className="inputInfoImage"
                                name="reseau"
                                type="text"
                                value={image.reseau}
                                onChange={(e) =>
                                  handleChangeInfoImage(
                                    e.target.value,
                                    index,
                                    indexImage,
                                    e.target.name
      ...
      <SelectInterSection dataInter={dataInter} setDataInter={setDataInter} />
