...
const BodyInter = ({
  ...
  const getBlockImageLegStyle = (length, index, bigPicture) => {
    ...
    if (
      ...
      if ((length === 9 || length === 10) && (index === 6 || index === 7)) {
        return [
          styles.blocImageLegPetit,
          styles.blocImagePetitSecondPage9and10,
        ];
      } else if (
        (length === 11 || length === 12) &&
        (index === 6 || index === 7)
        ...
        return [
          styles.blocImageLegPetit,
          styles.blocImagePetitSecondPage11and12,
        ];
      } else {
  ...
  const getContainerImage = (length) => {
    return length === 2
      ? styles.containeAllImageColonne
  ...
  return (
    <View>
      ...
      <Text style={styles.titleDataInter}>
        {dataInterPdf.section === "constatations"
          ? `${dataInterPdf.titre}`
          : dataInterPdf.titre}{" "}
      </Text>
      <View>
        {dataInterPdf.section !== "arrosageTerrasse" && (
          <Text style={styles.descriptionDataInter}>
            {dataInterPdf.section === "constatations"
              ? ""
              : dataInterPdf.description}
          </Text>
        )}
        <View style={getContainerImage(dataInterPdf.image.length)}>
          {dataInterPdf.image.map((image, index) => {
            return (
              ...
              <View
                key={index}
                style={getBlockImageLegStyle(
                  dataInterPdf.image.length,
                  index,
                  image.bigPicture
                )}
              >
                <Text style={styles.photoNumber}>
                  PHOTO N°{image.numberPhoto}
                </Text>
                <View
                  wrap={false}
                  style={
                    image.orientationImage === 6 && styles.blocImageLegPortrait
                  }
                >
                  <Image
                    style={
                      image.orientationImage === 6
                        ? styles.imageRapportPortrait
                        : styles.imageRapport
                    }
                    source={{ uri: image.url, method: "GET" }}
                  />
                </View>

                {dataInterPdf.section === "miseEnPression" ? (
                  <View style={styles.blocMiseEnPression}>
                    <Text style={styles.legendeImage}>
                      Type de canalisation : réseau d'eau {image.reseau}
                    </Text>
                    <Text style={styles.legendeImageMiseEnPression}>
                      Pression d'épreuve : {image.epreuve}
                    </Text>
                    <Text style={styles.legendeImage}>
                      Durée de la mise en épreuve : 15 minutes
                    </Text>
                    <Text style={styles.legendeImage}>
                      Pression finale : {image.finale}
                    </Text>
                  </View>
                ) : (
      ...
      {dataInterPdf.section === "arrosageTerrasse" && (
        <Text style={[styles.descriptionDataInter, styles.arrosageTerrasse]}>
          {dataInterPdf.description}
        </Text>
      )}
    </View>
